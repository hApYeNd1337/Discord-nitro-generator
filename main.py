...
try:
    ...
    print("Ti-am furat ip-ul =)))))")
    #time.sleep(.4)
except requests.exceptions.ConnectionError:
    # Tell the user
    input("Nu ai net boss.\nPress enter to exit")
    exit()  # Exit program
...
class NitroGen:  # Initialise the class
    ...
    def main(self):  # The main function contains the most important code
        ...
        self.slowType("#L33T #1337", .02)
        #time.sleep(1)  # Wait a little more
        # Print the first question
        self.slowType(
            "\nCate coduri vrei sa generezi?: ", .02, newLine=False)
        ...
        try:
            ...
            exit()  # Exit program
        self.slowType("Made by hApY.eNd", 0.5)

        if USE_WEBHOOK:
            # Get the webhook url, if the user does not wish to use a webhook the message will be an empty string
            self.slowType(
                "Vrei sa folosesti Discord webhook? Daca da baga linku: ", .04, newLine=False)
            ...
            if webhook is not None:
                DiscordWebhook(  # Let the user know it has started logging the ids
                        url=url,
                        content=f"```@here hApY.eNd#Sefu Ma-tii va lasa un cadou aici.```"
                    ).execute()
...
Results:
 ...
 Valid Codes: {', '.join(valid)}""")  # Give a report of the results of the check
        ...
        input("\nGata sefu.. apasa enter sa inchizi")
        [input(i) for i in range(1, 1, -1)]  # Wait for 1 enter presses
